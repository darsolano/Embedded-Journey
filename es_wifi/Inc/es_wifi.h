...
typedef enum {
  ES_WIFI_STATUS_OK             = 0,
  ES_WIFI_STATUS_REQ_DATA_STAGE = -10,
  ES_WIFI_STATUS_ERROR          = -20,
  ES_WIFI_STATUS_TIMEOUT        = -30,
  ES_WIFI_STATUS_IO_ERROR       = -40,
  ES_WIFI_STATUS_UNEXPECTED_CLOSED_SOCKET = -50,
  ES_WIFI_STATUS_MODULE_CRASH   = -60
} ES_WIFI_Status_t;
...
typedef enum{
	...
	MQTT_USER_PASSW,
	MQTT_CA_CERT_KEY
}ES_WIFI_MQTT_SEC_MODE_t;

typedef enum{
	TLS_NONE = 0,
	TLS_ROOTCA,
	TLS_MUTUAL
}ES_WIFI_TLS_SEC_MODE_t;
...
typedef struct {
  ...
  uint8_t            		*ClientID;
  ES_WIFI_MQTT_SEC_MODE_t	MQTTSecMode;
} ES_WIFI_AWS_Conn_t;
#endif
...
typedef struct {
  ...
  char*              Name;
  uint8_t            Backlog;
  ES_WIFI_TLS_SEC_MODE_t	TlsSecMode;
} ES_WIFI_Conn_t;
...
#if (ES_WIFI_USE_AWS == 1)
ES_WIFI_Status_t  ES_WIFI_StartMQTTClientConnection(ES_WIFIObject_t *Obj, ES_WIFI_AWS_Conn_t *conn);

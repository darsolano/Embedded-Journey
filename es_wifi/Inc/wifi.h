...
typedef enum {
  WIFI_TCP_PROTOCOL = 0,
  WIFI_UDP_PROTOCOL = 1,
  WIFI_UDPLITE_PROTOCOL = 2,
  WIFI_TLS_PROTOCOL = 3,
  WIFI_MQTT_PROTOCOL = 4
}WIFI_Protocol_t;
...
typedef enum {
  WIFI_STATUS_OK             = 0,
  WIFI_STATUS_ERROR          = -11,
  WIFI_STATUS_NOT_SUPPORTED  = -22,
  WIFI_STATUS_JOINED         = -33,
  WIFI_STATUS_ASSIGNED       = -44,
  WIFI_STATUS_TIMEOUT        = -55
} WIFI_Status_t;

typedef enum {
    WIFI_CRED_MODE_TLS = 0,
    WIFI_CRED_MODE_MQTT = 1
} WiFi_CredMode_t;

typedef struct {
	unsigned char 	*tls_ca_certs; 			/**< wifi module tls root certificate Socket option. */
...
typedef struct {
    char*    endpoint_url;   // e.g., "a3xxxx-ats.iot.us-east-1.amazonaws.com"
    uint16_t  port;           // Usually 8883 for AWS MQTT
...
WIFI_Status_t WIFI_OpenClientConnection(uint32_t socket, WIFI_Protocol_t type, ES_WIFI_TLS_SEC_MODE_t secmode,
...
WIFI_Status_t WIFI_SetCertificatesCredentials(WiFi_Tls_t *identity, WiFi_CredMode_t mode);

...
ES_WIFI_Status_t ES_WIFI_StartClientConnection(ES_WIFIObject_t *Obj,
	...
	if ((ret == ES_WIFI_STATUS_OK)
			&& (conn->Type == ES_WIFI_TCP_SSL_CONNECTION)) {
		sprintf((char*) Obj->CmdData, "P9=%d\r", conn->TlsSecMode);
		ret = AT_ExecuteCommand(Obj, Obj->CmdData, Obj->CmdData);
	}
...
/**
  * @brief  Configure and Start a AWS or any MQTT Client connection.
  * @param  Obj: pointer to the module handle
  * @param  conn: pointer to the connection structure
  * @retval Operation Status.
  */
ES_WIFI_Status_t ES_WIFI_StartMQTTClientConnection(ES_WIFIObject_t *Obj,
	...
	if (ret == ES_WIFI_STATUS_OK) {
		...
		if (ret == ES_WIFI_STATUS_OK) {
			...
			ret = AT_ExecuteCommand(Obj, Obj->CmdData, Obj->CmdData);
			if (ret == ES_WIFI_STATUS_OK) {
				sprintf((char*) Obj->CmdData, "P3=%d.%d.%d.%d\r",
												conn->RemoteIP[0],
												conn->RemoteIP[1],
												conn->RemoteIP[2],
												conn->RemoteIP[3]);
				ret = AT_ExecuteCommand(Obj, Obj->CmdData, Obj->CmdData);
				if (ret == ES_WIFI_STATUS_OK) {
					sprintf((char*) Obj->CmdData, "PM=0,%s\r",
							conn->PublishTopic);
					ret = AT_ExecuteCommand(Obj, Obj->CmdData, Obj->CmdData);
					if (ret == ES_WIFI_STATUS_OK) {
						sprintf((char*) Obj->CmdData, "PM=1,%s\r",
								conn->SubscribeTopic);
						ret = AT_ExecuteCommand(Obj, Obj->CmdData,
								Obj->CmdData);
						if (ret == ES_WIFI_STATUS_OK) {
							sprintf((char*) Obj->CmdData, "PM=2,%d\r",
									conn->MQTTSecMode);
							ret = AT_ExecuteCommand(Obj, Obj->CmdData,
									Obj->CmdData);
							if (ret == ES_WIFI_STATUS_OK) {
								sprintf((char*) Obj->CmdData, "PM=5,%s\r",
										conn->ClientID);
								ret = AT_ExecuteCommand(Obj, Obj->CmdData,
										Obj->CmdData);
								if (ret == ES_WIFI_STATUS_OK) {
									...
									if (ret == ES_WIFI_STATUS_OK) {
										sprintf((char*) Obj->CmdData, "P6=1\r");
										ret = AT_ExecuteCommand(Obj,
												Obj->CmdData, Obj->CmdData);
									}
								}
							}
						}
  ...
  do
  {
    ...
    if (ret == ES_WIFI_STATUS_OK)
    {
      if ((strstr((char *)Obj->CmdData, "[SOMA]")) && (strstr((char *)Obj->CmdData, "[EOMA]")))
      {
        if(strstr((char *)Obj->CmdData, "Accepted"))
        {
         //msg_debug("SOMA Accepted\n");
        }
        else if(!strstr((char *)Obj->CmdData,"[SOMA][EOMA]"))
        {
          msg_error("Bad MR syntax msg %s\n", Obj->CmdData);
...
ES_WIFI_Status_t ES_WIFI_CloseServerConnection(ES_WIFIObject_t *Obj, uint8_t socket)
{
  ...
  if (ret != ES_WIFI_STATUS_OK)
  {
    msg_error(" Can not select socket %s\n", Obj->CmdData);
    UNLOCK_WIFI();
    return ret;
  }
...
ES_WIFI_Status_t ES_WIFI_StopServerSingleConn(ES_WIFIObject_t *Obj, uint8_t socket)
{
  ...
  if (ret != ES_WIFI_STATUS_OK)
  {
	  msg_debug("Selecting socket failed: %s\n", Obj->CmdData);
    UNLOCK_WIFI();
    return ret;
  }
  ...
  if (ret != ES_WIFI_STATUS_OK)
  {
	  msg_debug("Stopping server failed %s\n", Obj->CmdData);
    UNLOCK_WIFI();
    return ret;
  }
  ...
  if (ret == ES_WIFI_STATUS_OK)
  {
    ...
    if (ret == ES_WIFI_STATUS_OK)
    {
      ...
      if (ret == ES_WIFI_STATUS_OK)
      {
        ...
        {
        	msg_debug("Send Data detect error %s\n", (char *)Obj->CmdData);
          ret = ES_WIFI_STATUS_ERROR;
        }
      }
      else
      {
        msg_debug("Send Data command failed\n");
      }
    }
    else
    {
      msg_debug("S2 command failed\n");
    }
  }
  else
  {
	  msg_debug("P0 command failed\n");
  }
    ...
    if (ret == ES_WIFI_STATUS_OK)
    {
      ...
      if (ret == ES_WIFI_STATUS_OK)
      {
        ...
        if (ret == ES_WIFI_STATUS_OK)
        {
          ...
          if (ret != ES_WIFI_STATUS_OK)
          {
            msg_debug("AT_RequestReceiveData failed or client disconnected\n");
          }
        }
        else
        {
        	msg_debug("Setting timeout failed\n");
        }
      }
      else
      {
    	  msg_debug("Setting requested len failed\n");
        *Receivedlen = 0;
      }
    }
    else
    {
      msg_debug("Setting socket for read failed\n");
      issue15++;
    }
  }
                                         ...
                                         uint8_t *IPaddr, uint8_t IpAddrLength, uint16_t *pPort)
{
  ...
  *Receivedlen = 0;
  ...
  if (ret == ES_WIFI_STATUS_OK)
  {
    ...
    {
      msg_debug("AT_RequestReceiveData overflow\n.");
      ret = ES_WIFI_STATUS_ERROR;
    }
    else
    {
                                    ...
                                    uint8_t* key)
{
  ...
  int caLength = strlen((char*)ca);
  int certificateLength = strlen((char*)certificate);
  ...
  if (ret == ES_WIFI_STATUS_OK && ca)
  {
    ...
    if (ret == ES_WIFI_STATUS_OK && certificate)
    {

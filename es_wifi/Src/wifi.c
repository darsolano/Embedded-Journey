...
WIFI_Status_t WIFI_OpenClientConnection(uint32_t socket, WIFI_Protocol_t type,
		ES_WIFI_TLS_SEC_MODE_t secmode, const uint8_t *ipaddr, uint16_t port,
		uint16_t local_port) {
	WIFI_Status_t ret = WIFI_STATUS_ERROR;
	ES_WIFI_Conn_t conn;

	// Initialize structure
	memset(&conn, 0, sizeof(ES_WIFI_Conn_t));

	conn.Number = (uint8_t) socket;
	conn.RemotePort = port;
	conn.LocalPort = local_port;
	switch (type) {
	case WIFI_TCP_PROTOCOL:
		conn.Type = ES_WIFI_TCP_CONNECTION;
		break;
	case WIFI_UDP_PROTOCOL:
		conn.Type = ES_WIFI_UDP_CONNECTION;
		break;
	case WIFI_TLS_PROTOCOL:
		conn.Type = ES_WIFI_TCP_SSL_CONNECTION;
		break;
	case WIFI_MQTT_PROTOCOL:
		conn.Type = ES_WIFI_MQTT_CONNECTION;
		break;
	default:
		return WIFI_STATUS_ERROR;
	}

	conn.TlsSecMode = secmode;
...
WIFI_Status_t WIFI_WaitServerConnection(uint32_t socket, uint32_t Timeout,
	...
	if (ES_WIFI_STATUS_OK != ret && ret != ES_WIFI_STATUS_TIMEOUT) {
		if (RemotePort) {
			*RemotePort = 0;
		}
		if ((RemoteIp != NULL) && (4 <= RemoteIpAddrLength)) {
		...
		return WIFI_STATUS_ERROR;
	} else if (ret == ES_WIFI_STATUS_TIMEOUT) {
...
WIFI_Status_t WIFI_SetCertificatesCredentials(WiFi_Tls_t *tls, WiFi_CredMode_t mode) {
    uint8_t credsFunction;
    uint8_t credSet;

    ES_WIFI_Status_t status = ES_WIFI_STATUS_ERROR;

    // Map your specific logic
    if (mode == WIFI_CRED_MODE_TLS) {
        credsFunction = 0;
        credSet = 0;
    } else if (mode == WIFI_CRED_MODE_MQTT) {
        credsFunction = 1;
        credSet = 2;
    } else {
        return WIFI_STATUS_ERROR; // Unsupported mode
    }

    if (tls->tls_ca_certs){
    	status = ES_WIFI_StoreCA(&EsWifiObj, credsFunction,
    			credSet, tls->tls_ca_certs, strlen((char*)tls->tls_ca_certs));
    }else{
    	tls->tls_ca_certs = NULL;
    }

    if (tls->tls_dev_cert && status == ES_WIFI_STATUS_OK){
    	status = ES_WIFI_StoreCertificate(&EsWifiObj, credsFunction,
    			credSet, tls->tls_dev_cert, strlen((char*)tls->tls_dev_cert));

    }else{
    	tls->tls_dev_cert = NULL;
    }

    if (tls->tls_dev_key && status == ES_WIFI_STATUS_OK){
    	status = ES_WIFI_StoreKey(&EsWifiObj, credsFunction,
    			credSet, tls->tls_dev_key, strlen((char*)tls->tls_dev_key));

    }else{
    	tls->tls_dev_key = NULL;
    }
    // Call the low-level API with the mapped values
//    ES_WIFI_Status_t status = ES_WIFI_StoreCreds(&EsWifiObj,
//                                                 credsFunction,
//                                                 credSet,
//                                                 tls->tls_ca_certs,
//                                                 tls->tls_dev_cert,
//                                                 tls->tls_dev_key);
    ...
    return (status == ES_WIFI_STATUS_OK) ? WIFI_STATUS_OK : WIFI_STATUS_ERROR;
}

/**
  * @brief  X-CUBE-AWS High-level wrapper for MQTT connection.
  * @param  config: Application-level configuration (endpoints, topics).
  * @retval 0 if successful, -1 if failed.
  */
WIFI_Status_t WIFI_MQTTIoTConnect(uint32_t socket, const uint8_t* ip_addr, const WiFi_MQTT_Config_t *config){
    if (config == NULL) return WIFI_STATUS_ERROR;

    /* 1. Map to X-CUBE-AWS specific connection structure */
    ES_WIFI_AWS_Conn_t aws_conn = {
        .Type 			= 	ES_WIFI_MQTT_CONNECTION,
        .Number 		= 	socket,                   // Standard Profile index
        .RemotePort 	= 	config->port,
        .MQTTSecMode	= 	(config->port == 1883)?MQTT_OPEN:MQTT_CA_CERT_KEY,// 1 = Secure MQTT for AWS IoT Core
        .ClientID 		= 	(uint8_t*)config->client_id,
        .PublishTopic 	= 	(uint8_t*)config->pub_topic,
        .SubscribeTopic = 	(uint8_t*)config->sub_topic,
	    .RemoteIP[0] = ip_addr[0],
	    .RemoteIP[1] = ip_addr[1],
	    .RemoteIP[2] = ip_addr[2],
	    .RemoteIP[3] = ip_addr[3]
    };

    /* 2. Execute connection through the hardware handle */
    // Note: EsWifiObj is the global handle usually defined in 'es_wifi_io.c'
    ES_WIFI_Status_t status = ES_WIFI_StartMQTTClientConnection(&EsWifiObj, &aws_conn);
